...
class Monitor:
    ...
    def _average_total_distance(self):
        ...
        for activities in self._activities[DRIVER].values():
            ...
            total_distance += manhattan_distance(activities[0].location,
                                                 activities[1].location) + \
                              manhattan_distance(activities[1].location,
                                                 activities[2].location)
            # divide it by number of drivers
            driver_count += 1
    ...
    def _average_ride_distance(self):
        ...
        driver_count = 0
        for activities in self._activities[DRIVER].values():
            # a driver has driven from pickup to dropoff
            # sum the ride distance of drivers
            drive_distance += manhattan_distance(activities[1].location,
                                                 activities[2].location)
            # divide by number of drivers
            driver_count += 1
            return drive_distance / driver_count
